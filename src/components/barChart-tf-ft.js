  ...
  const chartData = Object.keys(tf || {})
    ...
    ?.map((year) => {
      return {
        ...
        "Potential Bus Factor ": tf?.[year]?.future_tf.length,
      };
    });

  return (
    <div className="border p-4">
      <h3 className="dark:text-dark-tremor-content-strong text-lg font-medium text-tremor-content-strong">Current & Potential Bus Factor</h3>
      <p className="dark:text-dark-tremor-content text-tremor-default text-tremor-content">The chart below shows the current and potential (future) bus factor</p>
      ...
      <div className="flex flex-row items-center">
        <BarChart
          ...
          minValue={0}
          allowDecimals={false}
          categories={["Current Bus Factor", "Potential Bus Factor "]}
